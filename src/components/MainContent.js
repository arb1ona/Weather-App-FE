...
//         return { city, time_now, forcastDays }
//     }
//     // this helper function , receive the props from previous forcastConstruction
//     // we gonna construct the forcastComponent itself and send the properties that we have extracted 
//     createForcastComponent = (data) => {
//         return (<Forcast key={Math.random() * 100} city={data.city} time={data.time_now} forcastDays={data.forcastDays} />)
//     }
...
//         if (element.value === '')
//             return;
//         let locationName = element.value;

//         const url = `http:localhost:5000/forcasts/${locationName}/`
//         fetch(url)
//             .then(response => { return response.json() })
//             .then(data => {
//                 const date = new Date();
...
//                 this.forcasts.push({ city: locationName, time_now, consolidated_weather: data.consolidated_weather })
...
//                 </div>
//                 <div className="all-forcasts">
//                     {(this.state.allForcasts === undefined || this.state.allForcasts.length < 1) && (<h3>No Forecast to show</h3>)}
//                     {(this.state.allForcasts && this.state.allForcasts.length >= 1) && (
//                         allComponents.map(item => { return item; })
//                     )}
//                 </div>
//             </div>
//         )
